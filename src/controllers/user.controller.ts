...
const signup = async (req: Request, res: any, next: NextFunction) => {
  ...
  try {
    ...
    // Check if email was sent recently (within 20 minutes)
    if (user.lastVerificationEmailSent) {
      const timeSinceLastEmail = Date.now() - user.lastVerificationEmailSent.getTime();
      const minWaitTime = 20 * 60 * 1000; // 20 minutes in milliseconds

      if (timeSinceLastEmail < minWaitTime) {
        const remainingMinutes = Math.ceil((minWaitTime - timeSinceLastEmail) / 60000);
        return res
          .status(429)
          .json(new ApiError(
            429,
            `Please wait ${remainingMinutes} minute${remainingMinutes > 1 ? 's' : ''} before requesting another verification email`
          ));
      }
    }
